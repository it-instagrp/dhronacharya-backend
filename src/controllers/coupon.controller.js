...
//       return res.status(400).json({
//         message: "valid_from and valid_until must be valid dates (YYYY-MM-DD).",
//       });
//     }
...
//     // applicable_plan validation (spaces allowed)
//     if (!/^[A-Za-z0-9_\- ]+$/.test(applicable_plan)) {
//       return res.status(400).json({
//         message: "Invalid applicable_plan. Only alphanumeric, _, -, and spaces allowed.",
//       });
...
// /**
//  * User: Get coupons applicable to user's specific plan
//  */
// export const getUserApplicableCoupons = async (req, res) => {
//   const { plan_name } = req.query;
//   const userId = req.user?.id;
...
export const createCoupon = async (req, res) => {
  try {
    ...
    if (coupon_type === 'promotional' && expiry_hours) {
      ...
      valid_until = new Date(validFrom.getTime() + expiry_hours * 60 * 60 * 1000);
    }
  ...
  try {
    ...
    switch(coupon.coupon_type) {
      ...
      case 'retention':
        ...
        const hasActiveSubscription = false; // Implement your subscription check
        if (hasActiveSubscription) {
          validationError = 'Retention coupons are only for cancelled or expiring subscriptions.';
        }
        break;
  ...
  try {
    ...
    if (user_id) {
      ...
      switch(coupon.coupon_type) {
        ...
        case 'retention':
          // Implement retention eligibility check
          break;
      }
