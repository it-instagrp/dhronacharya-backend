...
const { ClassSchedule, User,Enquiry } = db;// enquiry added to get student and tuotor 
...
export const getAcceptedTutorsForStudent = async (req, res) => {
  try {
    const studentId = req.user.id;  // student logged in

    const accepted = await Enquiry.findAll({
      where: {
        receiver_id: studentId, // student is receiver
        status: "accepted"
      },
      include: [
        {
    ...
    // Extract tutors and remove duplicates by id
    const tutorsMap = new Map();
    accepted.forEach(item => {
      if (item.Sender) tutorsMap.set(item.Sender.id, item.Sender);
    });
...
export const getAcceptedStudentsForTutor = async (req, res) => {
  try {
    const tutorId = req.user.id;  // tutor logged in

    const accepted = await Enquiry.findAll({
      where: {
        sender_id: tutorId, // tutor is sender
        status: "accepted"
      },
      include: [
        {
    ...
    // Extract students and remove duplicates by id
    const studentsMap = new Map();
    accepted.forEach(item => {
      if (item.Receiver) studentsMap.set(item.Receiver.id, item.Receiver);
    });
