  ...
  confirmation: {
    email: ({ plan, price, duration, userName, couponCode, discountAmount }) => {
      ...
      const finalTotal = hasCoupon
        ? (priceNum * 1.18 - discountNum).toFixed(2)
        : totalBeforeDiscount;
...
- Team Dronacharya
    ...
    sms: ({ plan, price, couponCode, discountAmount }) => {
      ...
      return hasCoupon
        ? `🎉 You're subscribed to ${plan} on Dronacharya. Total Paid: ₹${finalTotal} (after ₹${discountNum.toFixed(2)} off with ${couponCode}). Enjoy premium access!`
